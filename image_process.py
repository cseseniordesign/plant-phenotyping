...
def draw_inflorescence(res, zoom_ratio):
	...
	for c in cnts:
		...
		cv2.putText(orig, "{:.1f}".format(real_width),
			(int(tltrX - 15), int(tltrY - 10)), cv2.FONT_HERSHEY_SIMPLEX,
			0.65, (0, 0, 0), 2)
		cv2.putText(orig, "{:.1f}".format(real_height),
...
def output_result(inf_width,inf_height,plant_height):
    s = "inflorescence width: "+str(inf_width)+" cm\ninflorescence height: "+str(inf_height)+" cm\nplant height: "+str(plant_height)+" cm"
...
def get_plant_height(image,zoom_ratio):
    ...
    cv2.putText(orig, "{:.2f}".format(real_plant_height),(int(stem_top[0]), int(stem_top[1])), cv2.FONT_HERSHEY_SIMPLEX,0.65, (0, 0, 0), 2)
...
if __name__ == "__main__":
	...
	if flowered == (True):
		...
		inf_height, inf_width = draw_inflorescence(inflorescence, zoom_ratio)
	else:
