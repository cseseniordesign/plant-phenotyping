...
for path in sorted(path_list):	
	...
	if 'npy' != plant_folder_name.split('.')[-1]:
		...
		preprocess.addArguments("-m", "schnablelab.CNN.Preprocess","hyp2arr", joined_path, plant_folder_name)
		dax.addJob(preprocess)
		nparr = File("%s.npy" % plant_folder_name)
		plant_name = plant_folder_name.split('_')[1]
		preprocess.uses(nparr, link=Link.OUTPUT, transfer=False, register=False)
		model = paths.file_paths['model']
		model_str = model.split('.')[0]
		prediction = File("%s.%s.prd.png" % (model_str, plant_folder_name))
		predict = Job("python3")
