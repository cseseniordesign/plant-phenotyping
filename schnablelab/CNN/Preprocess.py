...
def main():
    actions = (
        ...
        ('batch_hyp2arr', 'convert multiple hyperspectral images to numpy arrays')
    )
...
def batch_hyp2arr(args):
    p = OptionParser(batch_hyp2arr.__doc__)
    p.add_option('--pattern', default='Hyp*', help='hyper dir pattern for folders under dir')
    opts, args = p.parse_args(args)
    if len(args) == 0:
        sys.exit(not p.print_help())
    in_dir, out_dir, = args
    ...
    print('%s hyper folders found'%num_arrs)
    for hyp_dir in folders:
        hyp_dir_str = str(hyp_dir)
        index = hyp_dir_str[-3:]
        out_fn = "test"+index
        out_fn_path = out_path/out_fn
        cmd = "python3 -m schnablelab.CNN.Preprocess hyp2arr %s %s"%(hyp_dir_str,out_fn_path)
        os.system(cmd)
